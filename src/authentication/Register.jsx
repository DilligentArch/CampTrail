...
const Register = () => {
  ...
  const { createNewUser, setUser, updateUserProfile, handleSignInWithGoogle } = useContext(AuthContext);
    ...
    createNewUser(email, password)
      .then((result) => {
        ...
        updateUserProfile({ displayName: name, photoURL: photo })
          ...
          .catch((err) => {
            setError({ ...error, register: "Error updating profile" });
          });
      })
      .catch((err) => {
        setError({ ...error, register: "Error creating user" });
      });
  };
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-50">
      <div className="w-full max-w-md bg-white shadow-lg rounded-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Register Button */}
          <button type="submit" className="btn btn-primary w-full mt-6 p-3 rounded-md text-white bg-green-500 hover:bg-green-600 transition">
            Register
          </button>
        </form>
